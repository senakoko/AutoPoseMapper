...
def findWavelets(projections, pcaModes, omega0, numPeriods, samplingFreq, maxF, minF, numProcessors, useGPU):
    ...
    # Finding a set of dyadically spaced frequencies between the min freq and Nyquist freq.
    # Refer to formula C7 in Appendix C of Gordon's paper.
    dt = 1.0 / samplingFreq
    minT = 1.0 / maxF
    maxT = 1.0 / minF
    ...
    # Finding the amplitudes of the wavelet transform
    if useGPU>=0:
...
def fastWavelet_morlet_convolution_parallel(modeno, x, f, omega0, dt, useGPU):
    """
    Perform a parallel fast morlet convolution
    
    Paramters
    ---------
    modeno: the current pcaModes(from 0 to pcaModes-1)
    x: the timeseries projection data for the current pcaModes
    f: the dyadically spaced frequencies between the min freq and Nyquist freq
    omega0: non-dimensional parameter - default to 5
    dt: the samping period
    useGPU: indicates whether to run on GPU or CPU
    
    Returns
    -------
    amplitude: wavelet amplitudes (N x (pcaModes*numPeriods) )
    """
    if useGPU>=0:
        import cupy as np
        np.cuda.Device(useGPU).use()
    ...
    # Check if the timeseries is even or odd.
    # if it is odd the make even, otherwise do nothing
    if not N // 2:
    ...
    # Pad the projection,x, with zeros for the fourier transform
    x = np.concatenate([np.zeros(int(N / 2)), x, np.zeros(int(N / 2))], axis=0)
    M = N
    N = len(x)
    
    # Find the fourier frequency - formula C3 from Gordon's paper
    ...
    for i in range(L):
        # Find the wavelet Transform  - formula C2 from the paper
        m = (np.pi ** (-0.25)) * np.exp(-0.5 * (-Omegavals * scales[i] - omega0) ** 2)
        q = np.fft.ifft(m * xHat) * np.sqrt(scales[i])

        q = q[idx]
        # Find the wavelet Transform  - formula C5 from the paper
