...
def UniformFrames(clip, numframes2pick, start, stop, Index=None):
    ...
    Visual information of video is irrelevant for this method. This code is fast and sufficient
    (to extract distinct frames), when behavioral videos naturally covers many states.
    The variable Index allows passing on a sub index for the frames.
    """
    print(
...
def UniformFramescv2(cap, numframes2pick, start, stop, Index=None):
    ...
    print(
        "Uniformly extracting of frames from",
        round(start * nframes * 1.0 / cap_fps, 2),
        " seconds to",
        round(stop * nframes * 1.0 / cap_fps, 2),
        " seconds.",
    )
...
def KmeansbasedFrameselection(
    ...
    The video is extracted as a numpy array, which is then clustered with kmeans, whereby each frames is treated
    as a vector.
    Frames from different clusters are then selected for labeling. This procedure makes sure that the frames
    "look different", i.e. different postures etc. On large videos this code is slow.
    Consider not extracting the frames from the whole video but rather set start and stop to a period around
    ...
    if len(Index) >= numframes2pick:
        ...
        if np.ndim(frame0) == 3:
            ...
            ncolors = 1
        print("Extracting and down sampling...", nframes, " frames from the video.")
...
def KmeansbasedFrameselectioncv2(
    ...
    interesting behavior.
    Note: this method can return fewer images than numframes2pick.
    Attention: the flow of commands was not optimized for readability, but rather speed.
    This is why it might appear tedious and repetitive."""

    nframes = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    cap_fps = int(cap.get(cv2.CAP_PROP_FPS))
    nx, ny = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)), int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    ratio = resizewidth * 1.0 / nx
    if ratio > 1:
        raise Exception("Choice of resizewidth actually up samples!")
            ...
            if color:
                for counter, index in tqdm(enumerate(Index)):
                    ...
                    if frame is not None:
                        ...
                        )  # color not necessary; lack thereof improves speed.
                        if (
                            not allocated
